...
2. **Toon 选择** — 皮肤常用 toon02(偏红影)、脸可用 toon07(无影)、衣服/头发 toon01。各编号对照见 [[toon-贴图对照]]。
...
## 实测对照(范本 [[标准骨架范本-reika]],一个真实的 XPS→PMX 结果)

范本本身就是从 XPS rip 转来的(贴图名 `*_Base_Color.jpg`、`*_D.jpg`、`*_N.jpg`、`*_AO.jpg`、`*_Normal_OpenGL.jpg`),解析它能看到「转完未润色」的真实样子:

- **Sphere:0/32(全无)**、**描边:0/32(全关)**、Toon:32/32 全 toon01、両面:31/32、自身影:20/32。
- 34 张贴图里 **13 张法线(`_N`/`Normal`)、3 张 AO** —— 这些是 XPS/PBR 残留,**标准 MMD 用不到**,应在 PMX 里删掉引用(只留 base/diffuse)。
- 结论:自动转换**只保证能显示,不保证像 MMD**。卡通味(描边、toon、sphere)全靠手工补。润色清单见 [[材质-materials]]。
